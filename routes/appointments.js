...
router.post('/', function (req, res) {
	...
	let date  = moment.tz(appointment.date, "Asia/Colombo");
	let dateStr = date.format("MMM Do YYYY");
	messageapi.sendMessage(appointment.user.mobile,`Hey your appointment is confirmed on ${dateStr} for  ${appointment.slot.l} slot.Your appointment id is ${appointment.id}`).then((message) => {
		console.log(message);
	}).catch((err)=>{
		console.log(err);
	});
...
router.delete('/:id', function (req, res) {
   updateAppointment({id: req.params.id},{$set:{status : 0 }}).then(() => {
	...
	let date  = moment.tz(appointment.date, "Asia/Colombo");
        let dateStr = date.format("MMM Do YYYY");
	 messageapi.sendMessage(appointment.user.mobile,`Your appointment with appointment id  ${appointment.id} on ${dateStr} of ${appointment.slot.l} slot  is cancelled by doctor please reschedule it`).then((message) => {
                console.log(message);
        }).catch((err)=>{
                console.log(err);
       });
